    ...
    def __init__(self, in_channels, middle_channels, out_channels, downsample=False):
        ...
        if self.downsample:
            self.layer = nn.Sequential(
                ...
                conv1x1(in_channels, middle_channels, 2, 0), # downsampling from n to n / 2
                conv3x3(middle_channels, middle_channels, 1, 1), # ((n - 3 + 2) / 1) + 1 = n
...
class ResNet50_layer4(nn.Module):
    def __init__(self, num_classes = 10): # Hint : How many classes in Cifar-10 dataset?
        ...
        self.layer1 = nn.Sequential(
            nn.Conv2d(in_channels = 3, out_channels = 64, kernel_size = 7, stride = 2, padding = 3), # floor((32 - 7 + 6) / 2) + 1 = 16
            ...
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size = 3, stride = 2, padding = 1) # floor((16 - 3 + 2) / 2) + 1 = 8
        )
        self.layer2 = nn.Sequential(
            ResidualBlock(64, 64, 256, False),
            ResidualBlock(256, 64, 256, False),
            ResidualBlock(256, 64, 256, True) # 8 -> 4
        )
        self.layer3 = nn.Sequential(
            ...
            ResidualBlock(512, 128, 512, False),
            ResidualBlock(512, 128, 512, True) # 4 -> 2
            #########################################
        )
        ...
        self.fc = nn.Linear(in_features = 1024, out_features = num_classes) # Hint : Think about the reason why fc layer is needed
        self.avgpool = nn.AvgPool2d(kernel_size = 2, stride = 2) # 2 -> 1
